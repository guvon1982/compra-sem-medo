...
- **PRs #33 e #34 mergeados em `develop` (2026-06-21):** docs(claude). #33 registrou a sessao final (PRs #29-#32 + auditoria + helper normalizar); #34 corrigiu a receita do Kanban (PowerShell) + verificacao apos mover card. Codigo nao mudou.
- **Auditoria externa completa (2026-06-21):** revisao do projeto inteiro contra o `docs/Especificacao_do_Projeto_Final.md` (enunciado oficial). Conclusao: **6/6 requisitos funcionais e 4/4 itens de entrega cumpridos**, boas praticas de engenharia presentes (CI, branch protection, testes, camadas). Pontos levantados: `react-hook-form` declarado mas nao usado; URL da API fixa; README com "(a criar)"; sem rota 404; SDD inexistente; sem screenshots/Lighthouse no README; sem issues. Seguranca: ~90/100 para escopo academico, ~25/100 para producao real (json-server nao e backend de producao, sem auth, sem validacao server-side). Geraram-se 10 melhorias, aplicadas nos PRs #35-#37 + issues #38-#42.
- **PR #35 mergeado em `develop` (2026-06-21):** **react-hook-form de fato adotado no Cadastro** (antes era `useState` manual; a lib estava no `package.json` mas nao era usada — gap apontado pela auditoria). Usa `useForm` + `Controller` (e nao `register`) porque o `<Input>` e um componente controlado customizado — `Controller` e a forma recomendada pela lib nesse caso, sem alterar o `Input`. Validacoes preservadas 1:1 (nome >= 2 letras + unicidade RN2, categoria/unidade obrigatorias, preco via `parsePreco` > 0). Modo edicao pre-preenche via `reset()`; titulo do modal de exclusao usa `getValues("nome")` (leitura pontual — `watch()` no render dispara warning `react-hooks/incompatible-library`). 71 testes seguiram verdes.
- **PR #36 mergeado em `develop` (2026-06-21):** config + robustez. (a) **URL da API via env var:** `produtoService` le `import.meta.env.VITE_API_URL` com fallback `http://localhost:3000`; novo `.env.example` documenta a variavel (em dev nada muda). (b) **Rota catch-all 404:** nova pagina `src/pages/NotFound` (reusa Header + EmptyState) ligada a rota `*` em `App.jsx`, evitando tela branca em URL desconhecida. (c) **Metadata:** `index.html` ganhou `meta author` + tags Open Graph (`title/description/lang` ja existiam). (d) **Testes de componente:** +2 em `App.test.jsx` (renderiza 404; submit do form vazio dispara as validacoes do RHF). Total 71 -> 73.
...
- ✅ **73 testes automatizados verdes** (lint + test + build)
- ✅ 5 criterios da rubrica do professor cobertos (organizacao, tecnica, validacao, responsividade, versionamento)
- ✅ 34 PRs mergeados na `develop` com historico narrativo (sem squash gigante — CS7 do PRD atendido)
- ✅ **10 melhorias pos-auditoria aplicadas** (PRs #35-#37): react-hook-form, env var da API, rota 404, metadata, SDD, screenshots, Lighthouse, testes de componente, issues de rastreabilidade
...
- **Formularios com react-hook-form (`Cadastro/index.jsx`):** usar `useForm` + um `<Controller>` por campo, **nao** `register`. Motivo: o `<Input>` e um componente controlado customizado (recebe `value`/`onChange`, nao encaminha `ref`), e `Controller` e a forma recomendada pela lib para integrar componentes controlados de terceiros/proprios. Validacoes vao em `rules` (`required` ou `validate`). Pre-preenchimento (modo edicao) via `reset()`. Para leitura pontual de um campo no render (ex.: titulo de modal), usar `getValues("campo")` — **evitar `watch()` no corpo do render** (dispara warning de lint `react-hooks/incompatible-library`). Se outro form surgir no projeto, seguir o mesmo padrao.
- **URL base da API:** sempre via `import.meta.env.VITE_API_URL` com fallback para `http://localhost:3000` (ver `produtoService.js` e `.env.example`). Nunca cravar a URL no codigo — em dev o fallback cobre o json-server local; em producao a env var aponta para a API real. Variaveis expostas ao navegador precisam do prefixo `VITE_`.
...
**`npm run test:run` / `npm run build` falham no host Windows com `Cannot find module './rolldown-binding.win32-x64-msvc.node'`** (visto em 2026-06-21): o Vite 8 usa o `rolldown`, que tem binario nativo por plataforma. O `node_modules` deste projeto foi instalado para Linux (container), entao no host Windows o binario `win32-x64` nao existe e test/build quebram. O `npm run lint` funciona no host (ESLint e JS puro). **Solucao: rodar test e build dentro do Docker** (`docker compose exec -T app npm run test:run` / `... npm run build`) — que e o ambiente canonico, igual ao CI. Nao e bug do projeto; e mismatch de plataforma do `node_modules`.

**Rodar o Lighthouse de forma representativa:** medir sobre o **build**, nao sobre o `npm run dev`. No dev o codigo nao e otimizado e a Performance sai artificialmente baixa (visto: 34 no dev vs 70 no build). Receita: `docker compose exec -T app npm run build` e depois `docker compose exec -T -d app npm run preview -- --host 0.0.0.0 --port 4173`; abrir `http://localhost:4173` no Chrome → F12 → aba Lighthouse → Analyze page load (modo mobile).
